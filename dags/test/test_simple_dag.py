...
def write_text_file() -> None:
    with open(f"/tmp/test22.txt", "w") as fp:
        test = f"test22"
        ...
        s3.load_file(
            "/tmp/test22.txt", key=f"my-test-file{rand}.txt", bucket_name="airflow"
        )
...
t1 = PythonOperator(
    task_id="generate_and_upload_to_s322",
